...
namespace Tanneryd.BulkOperations.EF6.NET48.Tests.Tests.Insert
{
    ...
    public class BulkInsertAssignedGuidForeignKeyTests : BulkOperationTestBase
    {
        ...
        public void RecursiveInsert_ShouldSucceed_WhenBlogIdIsAlreadyAssignedOnPost()
        {
            using (var db = new UnitTestContext())
            {
                // Blog/Post use store-generated Guid identity; leave Ids unset so
                // SelectNewEntities still treats the post as new.
        ...
        public void RecursiveInsert_ShouldStillWork_WhenGuidForeignKeyIsEmpty()
        {
            using (var db = new UnitTestContext())
            {
                // Match the existing one-to-many child-insert pattern: leave BlogId
                // at default(Guid) so recursive insert copies it from Blog.
