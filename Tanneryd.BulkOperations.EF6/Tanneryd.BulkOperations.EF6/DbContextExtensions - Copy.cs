...
namespace Tanneryd.BulkOperations.EF6
{
    public static class DbContextExtensions
    {
        ...
        private static IList<T1> DoBulkSelectNotExisting<T1, T2>(DbContext ctx, BulkSelectRequest<T1> request)
        {
            ...
            if (keyMappings.Any())
            {
                ...
                foreach (var entity in items)
                {
                    ...
                    columnValues.AddRange(keyProperties.Select(p => GetPropertyValue(type, itemPropertByEntityProperty[p.Name], e, DBNull.Value)));
                    columnValues.Add(i++);
                ...
                using (var sqlDataReader = cmd.ExecuteReader())
                {
                    while (sqlDataReader.Read())
                    {
                        ...
                        foreach (var property in properties)
                        {
                            ...
                            SetPropertyValue(property, t2, val);
                        }
                    }
                }
            ...
            BulkInsertResponse response)
        {
            ...
            if (recursive)
            {
                ...
                foreach (var fkMapping in mappings.ToForeignKeyMappings)
                {
                    ...
                    // situation whee we would be interested in a collection type
                    ...
                    {
                        var navProperty = GetPropertyValue(t, navigationPropertyName, entity);
                        if (navProperty != null)
                        {
                            if (navPropertyType == null)
                                navPropertyType = GetPropertyValue(t, navigationPropertyName, entity).GetType();
                            foreach (var foreignKeyRelation in fkMapping.ForeignKeyRelations)
                            {
                                ...
                                var navPropertyKey = GetPropertyValue(t, foreignKeyRelation.ToProperty, entity);

                                if (navPropertyKey == null ||
                                    (isGuid && navPropertyKey == Guid.Empty) ||
                                    navPropertyKey == 0)
                                {
                                    var currentValue = GetPropertyValue(navPropertyType, foreignKeyRelation.FromProperty, navProperty);
                                    if ((isGuid && navPropertyKey != Guid.Empty) || 
                                        (!isGuid && currentValue > 0))
                                    {
                                        SetPropertyValue(foreignKeyRelation.ToProperty, entity, currentValue);
                                    }
                                    else
                                    {
                    ...
                    foreach (var modifiedEntity in modifiedEntities)
                    {
                        ...
                        foreach (var foreignKeyRelation in fkMapping.ForeignKeyRelations)
                        {
                            SetPropertyValue(foreignKeyRelation.ToProperty, e, GetPropertyValue(foreignKeyRelation.FromProperty, p));
                        }
                    }
                }
            }
            ...
            if (recursive)
            {
                ...
                foreach (var fkMapping in fkMappings)
                {
                    ...
                    foreach (var entity in entities)
                    {
                        if (isCollection)
                        {
                            var propertyCollection = GetPropertyValue(t, navigationPropertyName, entity);
                            ...
                            if (fkMapping.ForeignKeyRelations != null)
                            {
                                foreach (var navProperty in navProperties)
                                {
                                    foreach (var foreignKeyRelation in fkMapping.ForeignKeyRelations)
                                    {
                                        SetPropertyValue(foreignKeyRelation.ToProperty, navProperty,
                                            GetPropertyValue(t, foreignKeyRelation.FromProperty, entity));
                                    }
                            ...
                            else if (fkMapping.AssociationMapping != null)
                            {
                                ...
                                if (fkMapping.AssociationMapping.Source.EntityProperty.DeclaringType.Name == entity.GetType().Name)
                                {
                                    foreach (var navProperty in navProperties)
                                    {
                                        ...
                                        AddProperty(np, fkMapping.AssociationMapping.Source.TableColumn.Name, GetPropertyValue(t, fkMapping.AssociationMapping.Source.EntityProperty.Name, entity));
                                        AddProperty(np, fkMapping.AssociationMapping.Target.TableColumn.Name, GetPropertyValue((PropertyInfo) navPropertyType, fkMapping.AssociationMapping.Target.EntityProperty.Name, navProperty));
                                ...
                                else
                                {
                                    ...
                                    {
                                        dynamic np = new ExpandoObject();
                                        AddProperty(np, fkMapping.AssociationMapping.Source.TableColumn.Name, GetPropertyValue((PropertyInfo) navPropertyType, fkMapping.AssociationMapping.Source.EntityProperty.Name, navProperty));
                                        AddProperty(np, fkMapping.AssociationMapping.Target.TableColumn.Name, GetPropertyValue(t, fkMapping.AssociationMapping.Target.EntityProperty.Name, entity));
                                        navPropertyEntities.Add(np);
                                    }
                                }
                            }
                        ...
                        else
                        {
                            ...
                            if (navProperty != null)
                            {
                                foreach (var foreignKeyRelation in fkMapping.ForeignKeyRelations)
                                {
                                    SetPropertyValue(foreignKeyRelation.ToProperty, navProperty, GetPropertyValue(t, foreignKeyRelation.FromProperty, entity));
                                }
            ...
            BulkInsertResponse response)
        {
            ...
            if (pkColumnMappings.Length == 0)
            {
                ...
                if (entities[0] is ExpandoObject)
                {
                    foreach (var entity in entities)
                    {
                        ...
                        table.Rows.Add(properties.Select(p => GetPropertyValue(p.Name, e, DBNull.Value))
                            .ToArray());
                    }
                }
                else
                {
                    ...
                    {
                        var e = entity;
                        table.Rows.Add(props.Select(p => GetPropertyValue(t, p.Name, e, DBNull.Value))
                            .ToArray());
                    }
                }
                     ...
                     (pkColumnMappings[0].TableColumn.IsStoreGeneratedIdentity || pkColumnMappings[0].TableColumn.IsStoreGeneratedComputed))
            {
                ...
                if (entities[0] is ExpandoObject)
                {
                    ...
                    {
                        var e = (ExpandoObject)entity;
                        var pk = GetPropertyValue(pkProperty.Name, e);
                        if (pk == 0)
                ...
                else
                {
                    foreach (var entity in entities)
                    {
                        var pk = GetPropertyValue(t, pkProperty.Name, entity);
                ...
                if (newEntities.Count > 0)
                {
                    ...
                    if (newEntities[0] is ExpandoObject)
                    {
                        ...
                        foreach (var entity in newEntities)
                        {
                            var e = (ExpandoObject)entity;
                            var columnValues = new List<dynamic>();
                            columnValues.AddRange(properties.Select(p => GetPropertyValue(p.Name, e)));
                            columnValues.Add(i++);
                            table.Rows.Add(columnValues.ToArray());
                        }
                    }
                    else
                    {
                        ...
                        foreach (var entity in newEntities)
                        {
                            var e = entity;
                            var columnValues = new List<dynamic>();
                            columnValues.AddRange(properties.Select(p => GetPropertyValue(t, p.Name, e, DBNull.Value)));
                            columnValues.Add(i++);
                            table.Rows.Add(columnValues.ToArray());
                        }
                    }
                    ...
                    if (IntegerTypes.Contains(pkColumnType))
                    {
                        ...
                        {
                            SetPropertyValue(pkProperty.Name, newEntities[i], ids[i]);
                            ...
                            if (hasComplexProperties)
                            {
                                ...
                                {
                                    SetPropertyValue(pkProperty.Name, dict["#OriginalEntity"], ids[i]);
                                }
                            }
                        }
                    }
            ...
            SqlTransaction sqlTransaction)
        {
           ...
           //
            ...
            if (entities[0] is ExpandoObject)
            {
                ...
                {
                    var e = (ExpandoObject)entity;
                    var columnValues = properties.Select(p => GetPropertyValue(p.Name, e)).ToList();
                    columnValues.Add(i++);
                    table.Rows.Add(columnValues.ToArray());
                }
            }
            else
            {
                ...
                {
                    var e = entity;
                    var columnValues = properties.Select(p => GetPropertyValue(type, p.Name, e, DBNull.Value)).ToList();
                    columnValues.Add(i++);
                    table.Rows.Add(columnValues.ToArray());
                }
            }
        ...
        /// <returns></returns>
        private static dynamic GetPropertyValue(string propertyName, object instance, object def = null)
        {
            var t = instance.GetType();
            return GetPropertyValue(t, propertyName, instance, def);
        }

        private static dynamic GetPropertyValue(Type t, string propertyName, object instance, object def = null)
        {
            if (t.IsPrimitive) return instance;
            if (t == typeof(string)) return instance;

            PropertyInfo property = t.GetProperty(propertyName);
            return GetPropertyValue(property, instance, def);
        }

        private static dynamic GetPropertyValue(string propertyName, ExpandoObject instance)
        {
            var dict = (IDictionary<string, object>)instance;
            return dict[propertyName];
        }

        private static dynamic GetPropertyValue(PropertyInfo property, object instance, object def = null)
        {
        ...
        private static void SetPropertyValue(BulkPropertyInfo property, object instance, object value)
        {
            if (value == DBNull.Value) return;
