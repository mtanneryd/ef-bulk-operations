    ...
    public class BulkSelectTests : BulkOperationTestBase
    {
        ...
        public void ExistingEntitiesShouldBeSelectedOnSingleKeyUsingSimpleType()
        {
            using (var db = Factory.CreateDbContext())
            {
                ...
                for (int i = 0; i < 50; i++)
                {
                    Assert.AreEqual((object) expectedNumbers[i].Id, existingNumbers[i].Id);
                    Assert.AreEqual((object) expectedNumbers[i].ParityId, existingNumbers[i].ParityId);
                    Assert.AreEqual((object) expectedNumbers[i].UpdatedAt.ToString(CultureInfo.InvariantCulture), existingNumbers[i].UpdatedAt.ToString(CultureInfo.InvariantCulture));
                    Assert.AreEqual((object) expectedNumbers[i].UpdatedBy, existingNumbers[i].UpdatedBy);
                    Assert.AreEqual((object) expectedNumbers[i].Value, existingNumbers[i].Value);
                }
            }
        }
